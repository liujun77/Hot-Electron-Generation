...
FILE *open_file(FILE *fout, char *name, char *mode)
{
  ...
  if(!strcmpC(name,"inline"))   return stdin;   // stdin if name=="inline"
  if(!strcmpC(name,"terminal")) return stdout;  // stdout if name=="terminal"
  if(strcmpC(name,"none"))                      // no action if name=="none"
